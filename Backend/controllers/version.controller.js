...
export async function restoreVersion(req, res) {
  try {
    ...
    if (version.snapshot?.encodedState) {
      ...
      console.log(' Converting old format (Buffer) to base64');
      snapshotToSend = {
        schemaVersion: 1,
    ...
    //  Update Mindmap.snapshot (for persistence)
    ...
    try {
      ...
      console.warn(' Failed to backup:', backupErr.message);
    }
    ...
    //   BROADCAST TO REALTIME (This updates all clients via Yjs!)
    ...
    try {
      ...
      console.warn(' Restore saved but broadcast failed - clients need manual refresh');
    }
