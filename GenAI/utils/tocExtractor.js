...
function isRunning(text, freqMap) {
  return (
    (freqMap.get((text || "").trim().toLowerCase().slice(0, 45))?.size || 0) > 3
  );
}
...
function findBoundaries(pagesData) {
  ...
  for (let i = BACK_SCAN_START; i < total; i++) {
    ...
    const found = lines.some(
      (l) =>
        l.length >= 5 && l.length <= 40 && BACK_HEADINGS.some((p) => p.test(l)),
    );
    if (found) {
      backIdx = Math.max(0, i - 1);
      console.log(`[TOC] Back boundary: page ${pagesData[i].pageNum}`);
      break;
    }
  }
  ...
  for (let i = 0; i < FRONT_CUTOFF; i++) {
    const lines = (pagesData[i].lines || [])
      .slice(0, 8)
      .map((l) => l.text.trim());
    if (lines.some((l) => FRONT_HEADINGS.some((p) => p.test(l))))
      frontIdx = Math.max(frontIdx, i + 1);
  }
  return {
    startIdx: Math.max(0, frontIdx),
    endIdx: Math.min(total - 1, backIdx),
...
function detectScheme(entries) {
  ...
  console.log(
    "[TOC] Scheme:",
    order.map((f) => `${f.name}→L${levelOf[f.name]}`).join(", "),
    `| dotted base L${dottedBase}`,
  );

  return (e) => {
    if (e.type === "chapter" || e.type === "part")
      return levelOf["chapter"] ?? 1;
...
function adjustArabicLevels(entries) {
  for (let i = 1; i < entries.length; i++) {
    ...
    while (
      j >= 0 &&
      (entries[j].type === "arabic" || entries[j].type === "dotted")
    )
      j--;
    if (j >= 0) {
...
function buildTree(entries) {
  ...
  for (let i = 0; i < chapters.length; i++) {
    chapters[i].pageEnd =
      i + 1 < chapters.length ? (chapters[i + 1].pageStart ?? 9999) - 1 : 9999;
  }
...
const LIST_OF_X =
  /^(?:danh\s*(?:mục|sách)\s*(?:các\s*)?(?:hình|bảng|biểu(?:\s*đồ)?|sơ\s*đồ|đồ\s*thị|ảnh|từ\s*viết\s*tắt|chữ\s*viết\s*tắt|ký\s*hiệu|thuật\s*ngữ)|list\s+of\s+(?:figures?|tables?|charts?|abbreviations?|symbols?|illustrations?)|mục\s*lục\s+(?:hình|bảng))/i;

function findTOCPageText(pagesData) {
  let tocStart = -1;
  const searchLimit = Math.min(
    Math.max(15, Math.floor(pagesData.length * 0.2)),
    30,
  );
  for (let i = 0; i < Math.min(searchLimit, pagesData.length); i++) {
    const allLines = (pagesData[i].lines || [])
      .map((l) => l.text.trim())
      .filter(Boolean);
    if (
      allLines.slice(0, 8).some((l) => TOC_PAGE_MARKERS.some((p) => p.test(l)))
    ) {
      tocStart = i;
      break;
    }
  ...
  for (
    let pi = tocStart;
    pi < Math.min(tocStart + 15, pagesData.length);
    pi++
  ) {
    const pageLines = (pagesData[pi].lines || [])
      .map((l) => l.text.trim())
      .filter(Boolean);
    if (!pageLines.length) continue;

    if (pi > tocStart) {
      const isListOfX = pageLines
        .slice(0, 8)
        .some((l) => LIST_OF_X.test(l.trim()));
      if (isListOfX) {
        console.log(
          `[TOC] S1: stopped at page index ${pi} (danh mục hình/bảng → end of TOC)`,
        );
        break;
      }
      const codey = pageLines.filter(
        (l) =>
          /^[\{\}\[\]]/.test(l.trim()) ||
          /^"[\w_]+"\s*:/.test(l.trim()) ||
          /[{}]\s*,?\s*$/.test(l.trim()),
      ...
      if (codey >= 3) {
        console.log(
          `[TOC] S1: stopped at page index ${pi} (code/JSON detected)`,
        );
        break;
      }

      const LABEL_LINE =
        /^(?:\d{1,2}(?:\.\d{1,2})*[.\s)]|[A-Za-z][.)]\s|(?:chương|chapter|chap|phần|part|mục|bài)\s*[\dIVXivx]|X{0,3}(?:IX|IV|V?I{0,3})[.\s)])/i;
      const labeled = pageLines.filter((l) => LABEL_LINE.test(l.trim())).length;
      if (labeled < Math.max(2, pageLines.length * 0.25)) {
        console.log(
          `[TOC] S1: stopped at page index ${pi} (no structural labels → end of TOC)`,
        );
        break;
      }
    }
  ...
  console.log(
    `[TOC] S1: Found TOC page at index ${tocStart}, ${tocLines.length} lines (${skipPages.size} pages)`,
  );
...
function parseTOCPageJS(tocResult, pagesData) {
  ...
  const LABEL_START =
    /^(?:\d{1,2}(?:\.\d{1,2})*[\.\s]|[A-Za-z][\.]\s|(?:chương|chapter|chap|phần|part)\s+\d|(?:XI{0,2}|X|IX|VI{0,3}|V|IV|I{1,3})\s*[.\s]|(?:xi{0,2}|x|ix|vi{0,3}|v|iv|i{1,3})\s*[.\s])/i;
  ...
  for (const line of lines) {
    ...
    const prevHasPage = /\.{3,}\s*\d+\s*$|\s{4,}\d+\s*$|\d{1,4}\s*$/.test(
      prevLine,
    );
  ...
  for (const text of merged) {
    ...
    for (const re of TOC_ENTRY_RE) {
      ...
      const n = `${label}|${normalise(title)}`;
      if (seen.has(n)) break;
      seen.add(n);

      const isChapter =
        /^(chương|chapter|chap|phần|part|appendix|phụ lục)/i.test(text);
      const isUpperRoman =
        /^(XI{0,2}|X|IX|VI{0,3}|V|IV|I{1,3})$/.test(label) &&
      ...
      rawEntries.push({
        ...
        type,
        dots,
        pageStart: m[3] ? parseInt(m[3]) : null,
      });
      break;
    }
  }
  ...
  if (noSpaceCount > rawEntries.length * 0.4) {
    console.log(
      `[TOC] S1-JS: ${noSpaceCount}/${rawEntries.length} entries look like no-space PDF artifacts → LLM`,
    );
    return null;
  }
  const labeledTitles = new Set(
    rawEntries
      .filter((e) => e.type !== "nolabel")
      .map((e) => normalise(e.title)),
  );
  const dedupedEntries = rawEntries.filter(
    (e) => e.type !== "nolabel" || !labeledTitles.has(normalise(e.title)),
  );
  const scheme = detectScheme(dedupedEntries);
  const entries = adjustArabicLevels(
    dedupedEntries.map((e) => ({ ...e, level: scheme(e) })),
  );
  const chapters = buildTree(entries);
  console.log(
    `[TOC] S1-JS: ${chapters?.length || 0} chapters, ${rawEntries.length} entries`,
  );
  return chapters?.length >= 2
    ? {
        chapters: assignPageRanges(chapters, pagesData),
        method: "toc-page-js",
        skipPages,
      }
    : null;
...
##`;
  ...
  console.log(
    `[TOC-AI] Parsing TOC page (${tocPageText.length} chars) via LLM...`,
  );
  ...
  try {
    ...
    return {
      chapters: assignPageRanges(parsed, pagesData),
      method: "toc-page-llm",
    };
  } catch (err) {
    clearTimeout(tm);
...
export async function extractTOCWithFullText(pagesData, options = {}) {
  ...
  const uniqHeadings = headingLines
    .filter((line) => {
      const key = line
        .replace(/^\[p\d+\]\s*/, "")
        .toLowerCase()
  ...
  if (uniqHeadings.length < 3) {
    console.log(
      `[TOC-AI] only ${uniqHeadings.length} heading signals → skip full-text LLM (structureless)`,
    );
    return null;
  }
  ...
  const sampledText = Array.from(
    { length: Math.min(25, pagesData.length) },
    (_, i) => {
...
##`;
  ...
  console.log(
    `[TOC-AI] heading signals: ${uniqHeadings.length} | first: ${firstText.length}c | sampled: ${sampledText.length}c → LLM...`,
  );
  const ctl = new AbortController();
  ...
  try {
    ...
    console.log(
      `[TOC-AI] Got ${md.length} chars: ${md.slice(0, 250).replace(/\n/g, " ")}`,
    );

    const parsed = parseMarkdownTOC(md);
    if (!parsed?.length) {
      console.log("[TOC-AI] parseMarkdownTOC returned null");
      return null;
    }
...
export function assignPageRanges(chapters, pagesData) {
  ...
  for (const pg of pagesData)
    for (const l of pg.lines || []) {
      ...
      if (
        t.length >= 3 &&
        (l.isBold || (l.avgFont || 0) > (pg.bodyFont || 11) * 1.04)
      )
        idx.push({ norm: normalise(t), pageNum: pg.pageNum });
    }
  function findPage(title) {
    const nt = normalise(title);
    for (const { norm, pageNum } of idx) if (norm === nt) return pageNum;
    const tw = nt.split(/\s+/).filter((w) => w.length > 2);
    if (!tw.length) return null;
    for (const { norm, pageNum } of idx)
      if (tw.filter((w) => norm.includes(w)).length / tw.length >= 0.6)
        return pageNum;
    return null;
  }
  function walk(nodes, defS = 1, defE = lastPage) {
    for (let i = 0; i < nodes.length; i++) {
      const ch = nodes[i],
        next = nodes[i + 1];
      ch.pageStart = ch.pageStart ?? findPage(ch.title) ?? defS;
      const ns = next ? (next.pageStart ?? findPage(next.title)) : null;
      ch.pageEnd = ns ? ns - 1 : defE;
      if (ch.children?.length) walk(ch.children, ch.pageStart, ch.pageEnd);
    }
    return nodes;
  }
...
export function getChapterChunks(chapter, allChunks, topK = 10) {
  ...
  if (pageStart > 0 && pageEnd != null) {
    const strict = allChunks.filter(
      (c) =>
        c.pageNum != null && c.pageNum >= pageStart && c.pageNum <= pageEnd,
    );
    if (strict.length >= 1) pool = strict;
    else {
      const wide = allChunks.filter(
        (c) =>
          c.pageNum != null &&
          c.pageNum >= pageStart - 3 &&
          c.pageNum <= pageEnd + 3,
      );
      if (wide.length >= 1) pool = wide;
    }
  }
  const bySection = allChunks.filter(
    (c) =>
      c.sectionTitle &&
      normalise(c.sectionTitle).includes(normalise(title).slice(0, 20)),
  );
  if (bySection.length >= 1)
    pool = [
      ...new Map(
        [...pool, ...bySection].map((c) => [c.chunkIndex, c]),
      ).values(),
    ];
  const terms = title
    .toLowerCase()
    .split(/\s+/)
    .filter((w) => w.length > 2 && !/^(và|của|the|and|for)$/.test(w));
  return pool
    .map((c) => ({
      ...c,
      _s:
...
export function isTooSimilar(a, b, thr = 0.75) {
  const na = a.toLowerCase().trim(),
    nb = b.toLowerCase().trim();
  if (na === nb) return true;
  return 1 - levenshtein(na, nb) / Math.max(na.length, nb.length, 1) >= thr;
}
export function dedupNodes(candidates, ancestorTitles = [], thr = 0.78) {
  const kept = [];
  for (const c of candidates) {
    const title = c.text || c.title || "";
    if (!title) continue;
    if (ancestorTitles.some((a) => isTooSimilar(title, a, thr))) continue;
    if (kept.some((k) => isTooSimilar(title, k.text || k.title || "", thr)))
      continue;
    kept.push(c);
  }
  return kept;
